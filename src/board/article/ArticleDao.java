...
public class ArticleDao {
	...
	public Article getArticleById(int aid) {
		String sql = "SELECT a.*, m.nickname nickname, COUNT(l.aid) likeCnt FROM article a INNER JOIN `member` m ON a.mid = m.id INNER JOIN `like` l ON a.id = l.aid where a.id = ?";
	...
	public ArrayList<Article> getSearchedArticles(int searchFlag, String searchKeyword) {
		String sql1 = "SELECT a.*, m.nickname FROM article a INNER JOIN `member` m ON a.mid = m.id WHERE ";
		String sql2 = "";
		if(searchFlag == 1) {
